...
def load_jumia_items() -> list[dict]:
    ...
    if JUMIA_RAW_PATH.exists():
        log.info("items.json not found; converting raw Jumia data from %s", JUMIA_RAW_PATH)
        with open(JUMIA_RAW_PATH, encoding="utf-8") as f:
            raw = json.load(f)
        return _convert_raw_to_items(raw)

    raise FileNotFoundError(
        f"No Jumia data found. Run:\n"
        f"  python scripts/scrape_jumia.py\n"
        f"to scrape Jumia first, then re-run ingest."
    )
...
def _convert_raw_to_items(raw: list[dict]) -> list[dict]:
    ...
    for p in raw:
        ...
        enriched = " ".join(filter(None, [p.get("description", ""), reviews_blob])).strip()
        items.append({
            "id": p["id"],
            "name": p["name"],
            "category": p["category"],
